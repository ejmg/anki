...
* theory of computation
  :PROPERTIES:
  :ANKI_DECK: theory of computation
  :END:
** Formal Langs & Automata
*** Alphabet                                                 :WebberAdam2008:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :ANKI_NOTE_ID: 1599371874869
   :END:
**** Text
     An {{c1::alphabet}}, commonly denoted with a {{c1::$\Sigma$}} is {{c2::any finite set of symbols}}.
**** Extra
*** Strings                                                  :WebberAdam2008:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :END:
**** Text
     A {{c1::string}} is a {{c2::finite sequences}} of zero or more {{c2::symbols}}.
**** Extra

*** String concatenation                                     :WebberAdam2008:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :ANKI_NOTE_ID: 1599371948569
   :END:
**** Text
     The {{c1::concatenation}} of two strings $x$ and $y$ is the string containing {{c2::all the
     symbols, in order, of x}}, followed by the {{c2::symbols of y in order}}.
**** Extra
*** language                                                 :WebberAdam2008:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :ANKI_NOTE_ID: 1599372056296
   :END:
**** Text
     A {{c1::language}}, commonly described with a {{c2::set former}}, is a set of {{c3::strings}}
     over some {{c3::fixed alphabet}}.
**** Extra
*** Kleene Closure                                           :WebberAdam2008:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :ANKI_NOTE_ID: 1599371963695
   :END:
**** Text
     the {{c1::Kleene Closure}} of an {{c2::alphabet $\Sigma$}}, {{c1::denoted $\Sigma^{\ast}$}}, is the set of {{c2::all strings}}
     over {{c2::$\Sigma$}}.
**** Extra
*** Deterministic Finite Automata (DFA)                           :Wikipedia:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :ANKI_NOTE_ID: 1599372068571
   :END:
**** Text
     In theory of computation, a {{c1::Deterministic Finite Automata (DFA)}}, AKA
     {{c1::Deterministic Finite State Machine (DFSM)}}, is a {{c2::finite-state machine}} that
     accepts or rejects a {{c2::given string of symbols}} by running through a {{c2::state sequence
     uniquely}} determined by the {{c2::string}}.
**** Extra
*** 5-tuple (Formal Def of DFA)                              :WebberAdam2008:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :ANKI_NOTE_ID: 1599372566795
   :END:
**** Text
     A DFA has {{c1::five key parts}}, so we describe it mathematically as {{c1::a 5-tuple}}.

     A DFA /M/ is a {{c1::5-tuple}} {{c2::$M = (Q,\Sigma,\delta, q_{0}, F)$}} where:
     - {{c2::$Q$}} is the {{c3::finite set of states}}.
     - {{c2::$\Sigma$}} is the {{c4::alphabet, a finite set of symbols}}.
     - {{c2::$\delta \in (Q\times \Sigma \mapsto Q)$}} is the {{c5::transition function}}.
     - {{c2::$q_{0} \in Q$}} is the {{c6::start state}}.
     - {{c2::$F \subseteq Q$}} is the set of {{c7::accepting states}}.
**** Extra
*** Regular Language                                         :WebberAdam2008:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :ANKI_NOTE_ID: 1599372091619
   :END:
**** Text
     A {{c1::regular language}}, denoted {{c1::$L$}} is one that is {{c2::$L(M)$}}, i.e. {{c2::accepted}}, for some DFA
     $M$.
**** Extra
*** Extended transition function                             :WebberAdam2008:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :ANKI_NOTE_ID: 1599375315170
   :END:
**** Text
     Where {{c1::$\delta \in (Q \times \Sigma \mapsto Q)$}} is the {{c1::transition function}} for some DFA
     $M = (Q, \Sigma, \delta, q_{0}, F)$, the {{c2::extended transition function $\delta^{\ast}$}} can be
     defined as:

     \begin{align*}
     {{c3::\delta^{\ast}(q, \epsilon) &= q}} \\
     {{c3::\delta^{\ast}(q, xa) &= \delta(\delta^{\ast}(q, x), a)}}
     \end{align*}

     where $x$ is any string in $\Sigma^{\ast}$ and $a$ is the last symbol.

     The {{c2::extended transition function}} is a recursive function that tells which{{c1:: state $q$ that the
     DFA results in}}, starting from {{c3::$q$}} and making the sequence of {{c3::$\delta$ transitions on the
     symbols in $x$}}.
**** Extra
*** Language Accepted by DFA                                 :WebberAdam2008:
   :PROPERTIES:
   :ANKI_NOTE_TYPE: Cloze
   :END:
**** Text
     {{c1::$L(M)$}} denotes that a {{c2::language is accepted by some DFA, $M$,}} which is {{c1::$L(M) = {x \in \Sigma^{\ast} \mid
     \delta^{\ast}(q_{0}, x) \in F}$}}.
**** Extra
* rust
  :PROPERTIES:
  :ANKI_DECK: rust
...
* javascript
  :PROPERTIES:
  :ANKI_DECK: javascript
  :END:
** modern js
*** scope in js
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592866577297
    :END:
**** Front
     What are the two scopes in JS?
**** Back
     ...
     2. *Global*: variables are created as members of the *global object*, that is, they are given global
        scope, when they are not declared inside a function

     #+begin_src js
       function getDate() {
         // date is only accessible here within function scope
         var date = new Date();
         return date;
       }
       // works just fine
...
*** hoisting exercise
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793777954
    :END:
**** Front
     What does hoisting transform the following code into?
     #+begin_src js
...
*** let hoisting
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862871171
    :END:
**** Front
     Are =let= variables hoisted?
**** Back
     *Yes, all declarations in JS are hoisted*. However, accessing a variable that was declared with
     =let= /before the engine evaluates it's value at its location of declaration/ will return a
     =ReferenceError=, i.e. =let= variables only get initialized when *their lexical binding (assignment)
     is evaluated* by the JS engine.
*** const hoisting
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862871497
    :END:
**** Front
     Are =const= variables hoisted?
**** Back
     *Yes, all declarations in JS are hoisted*. However, accessing a variable that was declared with
     =const= /before the engine evaluates it's value at its location of declaration/ will return a
     =ReferenceError=, i.e. =const= variables only get initialized when *their lexical binding
     (assignment) is evaluated* by the JS engine.
*** temporal deadzone def
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Cloze
    :ANKI_NOTE_ID: 1592862871572
    :END:
**** Text
     the {{c1::*Temporal Deadzone*}} is the {{c2::time between variable declaration and
     initialization where they cannot be accessed}}. This especially concerns objects declared with
     {{c3::=const= and =let}}=, as accessing either {{c3::*before* their declaration}} will return a
     =ReferenceError=.
**** Extra
*** var scope
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793778455
    :END:
**** Front
     what is the scope of =var=?
**** Back
     *function scoped*. available throughout the entire function it is declared within.
*** var temporal dead zone ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862871746
    :END:
**** Front
...
**** Back
     ...
     =var= declared variables are hoisted, as are all declarations, but, unlike =const= and =let=, =var= is
     initialized with =undefined= before its evaluation by the JS engine.
*** let scope
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793778536
    :END:
**** Front
     what is the scope of =let=?
**** Back
     *block scoped*. it is available within the set of statements between the ={}='s you declared it
     within. think rust.
*** let temporal dead zone ex 1
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862871922
    :END:
**** Front
     what will be the result of the following snippet?
     #+begin_src js
     ...
     #+end_src
**** Back
     it will yield a =ReferenceError= because you *cannot access =let= declared objects before their
     lexical binding*, i.e. their declaration. thus, the call to =console.log(a)= occurs during the
     *temporal deadzone* of the variable =a=.
*** let temporal dead zone ex 2
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862872026
    :END:
**** Front
     what will be the result of the following snippet?
     #+begin_src js
...
**** Front
     ...
     #+end_src
**** Back
     it will log =6=. =let= variables can be reassigned and *are not* immutable.
*** let re-declaration ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862872422
    :END:
**** Front
     what will be the result of the following snippet?
     #+begin_src js
     ...
     #+end_src
**** Back
     =SyntaxError=, =let= variables *cannot be re-declared*.
*** const scope
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793778632
    :END:
**** Front
     what is the scope of =const=?
**** Back
     *block scope.* it is available within the set of statements between the ={}='s you declared
     it. think rust.
*** const temporal dead zone ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862872597
    :END:
**** Front
     what will be the result of the following snippet?
     #+begin_src js
     ...
     #+end_src
**** Back
     it will yield a =ReferenceError= because you *cannot access =const= declared objects before their
     lexical binding and evaluation*, i.e. their initialization. thus, the call to =console.log(a)=
     occurs during the *temporal deadzone* of the variable =a=.
*** const decl w/o init ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862872697
    :END:
**** Front
     what will be the result of the following snippet?
     #+begin_src js
     ...
     #+end_src
**** Back
     the snippet will yield a =SyntaxError= because you cannot declare a =const= variable without
     initialization.
*** const re-assignment ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862872799
    :END:
**** Front
     what will be the result of the following snippet?
     #+begin_src js
     ...
     #+end_src
**** Back
     =TypeError= as =const= assignments are *immutable*.
*** const re-declaration ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862872923
    :END:
**** Front
     what will be the result of the following snippet?
     #+begin_src js
     ...
     #+end_src
**** Back
     =SyntaxError= as =const= variables cannot be re-declared.
*** const immutability ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862873247
    :END:
**** Front
     what will be the result of the following snippet?
     #+begin_src js
     ...
     #+end_src
**** Back
     =person.name= will become ='Kim Kardashian West'.= This is totally legal because the =cont=
     /declaration is on the object/ and *does not apply to an object's fields*. Thus the action does not
     count as reassignment, but assigning =person= to ={}= would.
*** var vs let
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793778805
    :END:
**** Front
     differences between =var= and =let=
**** Back
     Both allow variables to be referenced before /initialization/ and both allow for /re-assignment/.

     Their differences include:
     - =var=:
       + *function scope*
       + =undefined= when accessed after declaration but without initialization
     - =let=:
       + *block scope*
       + =ReferenceError= when accessed before declaration;
       + *cannot be re-declared*. once a variable name is declared with =let=, it cannot be re-declared
         in that scope.
*** let vs const
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793778905
    :END:
**** Front
     differences between =let= and =const=
**** Back
     Both are *block scope* and will throw *ReferenceError* when referenced before evaluation by the JS
     engine. Both cannot be *re-declared*.

     - =let=:
       + can declare without initialization
       + can re-assign a value
     - =const=:
...
*** dot notation
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793779029
    :END:
**** Front
     add the field =name= with the value =ejmg= to the object =user= using *dot notation*
**** Back
     #+begin_src js
     user.name = "ejmg";
     #+end_src
*** obj literal notation
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793779158
    :END:
**** Front
     using *object literal notation*, assign =ejmg= and =kcmo= to the fields =name= and =local= to new object =user=
**** Back
     #+begin_src js
       const user = {
         name: "ejmg",
...
**** Back
     #+begin_src js
       const { name, local, handle } = user;
     #+end_src
*** arr destructuring
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793779806
    :END:
**** Front
     use *destructuring* on the following array:
     #+begin_src js
       const arr = ["elias", "kcmo", "ejmg"];
     #+end_src
**** Back
     #+begin_src js
       const [ name, local, handle ] = arr;
     #+end_src
*** renamed destructuring
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592793780082
    :END:
**** Front
     rename the fields in the object below to =name= and =local= while destructuring:
     #+begin_src js
       const user = {
         n: "elias",
         l: "kcmo",
...
**** Back
     #+begin_src js
       const {n: name, l: local } = user;
     #+end_src
*** obj destructure and default arguments
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862874425
    :END:
**** Front
     improve the following function signature using *object destructuring* and *default arguments*:
     #+begin_src js
...
**** Back
     #+begin_src js
       // what are considered good default args is obv subjective
       function fetchRepos({ language='All', 
                             minStars=0, 
                             maxStars='', 
                             createdBefore='', 
                             createdAfter='' }) {
     ...
     #+end_src
*** array destruct and function params
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862874699
    :END:
**** Front
     using *array destructuring* to improve the handling of the following promise:
     #+begin_src js
     ...
     #+end_src
**** Back
     There are multiple improvements here:
     1. we could destruct the fields out of =data=:
        #+begin_src js
          // ...
          .then(function (data) {
           ...
           // ...
        #+end_src
     2. better yet, why not destruct the array *inside the parameters* using an arrow function:
        #+begin_src js
         // ...
         then(([ profile, repos ]) => {
         // ...
        ...
        #+end_src
*** shorthand properties def
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862874892
    :END:
**** Front
     what are *shorthand properties*?
**** Back
     whenever you have a variable that matches the name of a property on an
     object, you can omit the property name when constructing an instance of that object.
*** shorthand properties ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862875246
    :END:
**** Front
     what is the shorthand property equivalent to the object returned by =formatMessage=?
     #+begin_src js
...
**** Back
     ...
     #+end_src
*** shorthand methods def
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862875322
    :END:
**** Front
     what are *shorthand method names*?
**** Back
     when a function is a property on an object, that makes it a *method*. As
     of ES6, when constructing an object that has a function as a field you can now omit the
     =function= keyword entirely.
*** shorthand methods ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862875496
    :END:
**** Front
     what is the shorthand equivalent to the object returned by =formatMessage=?
     #+begin_src js
       function formatMessage (name, id, avatar) {
           return {
               name,
...
**** Back
     ...
     #+end_src
*** computed properties def
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862875622
    :END:
**** Front
     What are *computed properties*?
**** Back
     as of ES6, you can have expressions that get evaluated as properties on an object when
     surrounded by =[]='s. this means that the =key=, i.e. property name, of an object can be evaluated,
     with the result being the actual name of the field:
     #+begin_src js
       const forever = "wu";
       const tang = "forever";
       const truth = {
...
*** computed properties ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862875772
    :END:
**** Front
     What object is returned by the following function?
     #+begin_src js
       function confusing_var_names(bar, buz, foo, baz) {
         return {
           [bar]: buz,
           baz: foo,
         }
       }
       const obj = confusing_var_names("buz", "foo", "bar", "baz");
     #+end_src
**** Back
     #+begin_src js
       {
     ...
     #+end_src
*** template literals ex
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862875926
    :END:
**** Front
     refactor the following function to use template literals:
     #+begin_src js
       function hello (name, email) {
         return "<h1>hello, " + name + "</h1>" +
           "<p>please check your email " + email + " to continue.</p>";
       }
     #+end_src
**** Back
     #+begin_src js
...
*** function expression
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862876249
    :END:
**** Front
     what is the function expression declaration equivalent for the following function?
     #+begin_src js
      function add (x,y) {
          return x + y;
      }     
...
*** arrow functions
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862876448
    :END:
**** Front
     What is the arrow function expression declaration equivalent for the following function?
     #+begin_src js
       function add (x,y) {
           return x + y;
       }
...
**** Back
     ...
     #+end_src
*** difference between function expr and declar
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :ANKI_NOTE_ID: 1592862876674
    :END:
**** Front
     What is the difference between a function expression, i.e. a function defined inside an
     expression, ex:
     #+begin_src js
     ...
     #+end_src
     and a normal function declaration? ex:
     #+begin_src js
...
**** Back
     - function expressions are *not hoisted*:
       ...
       #+end_src
     - function expressions allow the *omission of function name*
       + this allows the creation of *anonymous functions*
         * when bound to a variable, the variable becomes the *implicit name*:
           #+begin_src js
           ...
           #+end_src
         * critical to understand the above function declaration is technically an anonymous
           function still, however.
       + this allows function expressions to be used as *Immediately Invoked Function Expressions
         (IIFE)*'s
         #+begin_src js
           (function () {
         ...
         #+end_src
*** implicit returns (multi param)
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :END:
**** Front
     Write the equivalent arrow function using implicit returns:
     #+begin_src jsx
       function add(x,y) {
           return x + y;
       }     
...
**** Back
     #+begin_src jsx
       const add = (x, y) = x + y;
     #+end_src
*** implicit returns (single param)
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :END:
**** Front
     refactor the chaining using implicit returns:
     #+begin_src js
       // stuff
       return fetch('https://github.com/user_foo')
         .then((response) => {
           response.data
         });
     #+end_src
**** Back
     ...
     #+end_src
     note:
     - arrow functions like those above can drop the =()= around the parameters when there is only
       parameter
*** arrow functions and context
    ...
    :END:
**** Front
     how do arrow functions effect context and scope?
**** Back
     *they don't*, which is why they are so useful. arrow functions are *lexically scoped*, which means
     they take on their /surrounding scope/ rather than introducing their own new lexical scope.
*** differentiating arrow function bodies and implicit returns
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
    :END:
**** Front
...
**** 4 props of a transaction
     :PROPERTIES:
     :ANKI_NOTE_TYPE: Basic
     :ANKI_FAILURE_REASON: deck was not found: intro to database systems
     :END:
***** Front
