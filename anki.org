...
**** Back
     ...
     - =let=:
       + *block scope*
       + =ReferenceError= when accessed after declaration but without initialization
*** let vs const
    :PROPERTIES:
    :ANKI_NOTE_TYPE: Basic
...
**** Back
     Both are *block scope* and will throw *ReferenceError* when used without initialization.
